...
/**
 * An extended class of {@link XmppConnectionFactoryBean} with support for Reconnection and
 * connection draining implementation. It reconnects in all scenarios that the FCM xmpp server
 * disconnects (either via the client or the server). It also implements a Back off strategy as
 * recommended by google when connection draining is reported by the FCM Server. The FCM server
 * reports connection draining as a message before closing the connection. This needs to set in the
 * Message receiver via the {@link #setIsConnectionDraining(Boolean)}.
 *
...
public class ReconnectionEnabledXmppConnectionFactoryBean extends XmppConnectionFactoryBean
  ...
  public void start() {
    ...
    connection.addStanzaInterceptor(
        stanza -> {
           ...
           * connection with a presence unavailable message as can be seen in {@link
           * XMPPTCPConnection.PacketReader#parsePackets()} {@code // We received a closing stream
           * element from the server without us // sending a closing stream element first. This
           * means that the // server wants to terminate the session, therefore disconnect // the
   ...
   * the XMPP server).
   *
   * <p>Note that the connection is likely not yet authenticated and therefore only limited
   * operations like registering an account may be possible.
   *
  ...
  public void connectionClosed() {
    log.info(
        "Connection closed. The current connectionDraining flag is: {}."
            + "\nThe current Presence unavailable flag is {}",
  ...
  /**
   * Notification that the connection was closed due to an exception. When abruptly disconnected it
   ...
   *
   * <p>Note: This method is only called if {@link
   * ReconnectionManager#isAutomaticReconnectEnabled()} returns true, i.e. only when the
   * reconnection manager is enabled for the connection.
   *
  ...
  /**
   * An attempt to connect to the server has failed. The connection will keep trying reconnecting to
