...
package org.radarbase.appserver.microservices.task
...
fun main(args: Array<String>) {
    val logger = LoggerFactory.getLogger("org.radarbase.appserver.microservices.task.TaskMicroservice")

    logger.info("Starting Task Service")

    val config = try {
        ConfigLoader.loadConfig<TaskServiceConfig>(
            listOf(
                "microservices/task-service/src/main/resources/task-service.yml",
                "/etc/task-service/task-service.yml",
            ),
            args,
        ).withEnv()
    } catch (_: IllegalArgumentException) {
        logger.error("No configuration file (task-service.yml) found. Exiting...")
        exitProcess(1)
    }
