...
import org.radarbase.appserver.microservices.contract.utils.Env.TASK_HIBERNATE_DIALECT
import org.radarbase.appserver.microservices.contract.utils.Env.TASK_JDBC_DRIVER
import org.radarbase.appserver.microservices.contract.utils.Env.TASK_JDBC_PASSWORD
import org.radarbase.appserver.microservices.contract.utils.Env.TASK_JDBC_URL
...
data class TaskDbConfig(
    ...
    fun withEnv(): TaskDbConfig  = this
        .copyEnv(TASK_JDBC_URL) {
            copy(jdbcUrl = it)
        }
        .copyEnv(TASK_JDBC_USERNAME) {
            copy(username = it)
        }
        .copyEnv(TASK_JDBC_PASSWORD) {
            copy(password = it)
        }
        .copyEnv(TASK_HIBERNATE_DIALECT) {
            copy(hibernateDialect = it)
        }
        .copyEnv(TASK_JDBC_DRIVER) {
            copy(jdbcDriver = it)
        }
