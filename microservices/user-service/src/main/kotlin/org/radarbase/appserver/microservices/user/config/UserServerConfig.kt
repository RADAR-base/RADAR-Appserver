...
data class UserServerConfig(
    /** Base URL to serve data with. This will determine the base path and the port. */
    val baseUri: URI = URI.create("http://user-service:9013"),
    /**
    ...
    fun withEnv(): UserServerConfig = this
        .copyEnv(Env.USER_SERVICE_BASE_URL) {
            try {
                copy(baseUri = URI.create(it))
            } catch (e: Exception) {
                logger.error("Not a valid url from env APPSERVER_USER_SERVICE_BASE_URL", e)
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(UserServerConfig::class.java)
    }
}
