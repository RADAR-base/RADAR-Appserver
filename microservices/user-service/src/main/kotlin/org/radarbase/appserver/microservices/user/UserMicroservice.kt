...
fun main(args: Array<String>) {
    val logger = LoggerFactory.getLogger("org.radarbase.appserver.microservices.user.UserMicroservice")
    ...
    val config = try {
        ConfigLoader.loadConfig<UserServiceConfig>(
            listOf(
                "microservices/user-service/src/main/resources/user-service.yml",
                "/etc/user-service/user-service.yml",
            ),
            args,
        ).withEnv()
    } catch (_: IllegalArgumentException) {
        logger.error("No configuration file (user-service.yml) found. Exiting...")
        exitProcess(1)
    }
