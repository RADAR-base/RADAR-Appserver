...
data class GithubServerConfig(
    /** Base URL to serve data with. This will determine the base path and the port. */
    val baseUri: URI = URI.create("http://github-service:9011"),
    /**
    ...
    fun withEnv(): GithubServerConfig = this
        .copyEnv(Env.GITHUB_SERVICE_BASE_URL) {
            try {
                copy(baseUri = URI.create(it))
            } catch (e: Exception) {
                logger.error("Not a valid url from env APPSERVER_GITHUB_SERVICE_BASE_URL", e)
                this@GithubServerConfig
            }
        }
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(GithubServerConfig::class.java)
    }
}
