...
data class ProjectServerConfig(
    /** Base URL to serve data with. This will determine the base path and the port. */
    val baseUri: URI = URI.create("http://project-service:9010"),
    /**
    ...
    fun withEnv(): ProjectServerConfig = this
        .copyEnv(Env.PROJECT_SERVICE_BASE_URL) {
            try {
                copy(baseUri = URI.create(it))
            } catch (e: Exception) {
                logger.error("Not a valid url from env APPSERVER_PROJECT_SERVICE_BASE_URL", e)
                this@ProjectServerConfig
            }
        }
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(ProjectServerConfig::class.java)
    }
}
