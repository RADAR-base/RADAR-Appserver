 ...
 * ProtocolFetcherStrategy to fetch protocol-related data and uses caching
 * techniques to minimize redundant network calls.
 *
 * @param protocolFetcherStrategy A [ProtocolFetcherStrategy][org.radarbase.appserver.microservices.core.service.github.protocol.ProtocolFetcherStrategy] interface implementation used for
...
class DefaultProtocolGenerator @Inject constructor(
     ...
     */
    @Transient
    private var cachedProjectProtocolMap: CachedMap<String, Protocol> = CachedMap(
     ...
     */
    override suspend fun retrieveAllProtocols(): Map<String, Protocol> {
        return try {
     ...
     */
    @Throws(IOException::class)
    private suspend fun forceGetProtocolForSubject(subjectId: String): Protocol? {
        try {
