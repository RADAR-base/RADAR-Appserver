...
data class ProtocolServerConfig(
    /** Base URL to serve data with. This will determine the base path and the port. */
    val baseUri: URI = URI.create("http://protocol-service:9012"),
    /**
    ...
    fun withEnv(): ProtocolServerConfig = this
        .copyEnv(Env.PROTOCOL_SERVICE_BASE_URL) {
            try {
                copy(baseUri = URI.create(it))
            } catch (e: Exception) {
                logger.error("Not a valid url from env APPSERVER_PROTOCOL_SERVICE_BASE_URL", e)
                this@ProtocolServerConfig
            }
        }
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(ProtocolServerConfig::class.java)
    }
}
