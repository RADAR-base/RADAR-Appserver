...
package org.radarbase.appserver.microservices.core.utils.cache

import kotlinx.coroutines.sync.Mutex
import kotlinx.coroutines.sync.withLock
import org.radarbase.appserver.microservices.core.utils.cache.deps.CustomThrowingFunction
    ...
    private inner class LockedResult(
        ...
        suspend fun getOrCompute(): O = computeLock.withLock {
            ...
            try {
                ...
                return function.applyWithException(input).also { computedResult ->
                    reference = SoftReference(
                        Result.Success(
                            computedResult,
                            Instant.now().plus(cacheTime),
                        ),
                    )
                }
            } catch (ex: Exception) {
