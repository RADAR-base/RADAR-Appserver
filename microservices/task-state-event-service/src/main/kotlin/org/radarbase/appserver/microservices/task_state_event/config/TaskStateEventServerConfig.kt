...
data class TaskStateEventServerConfig(
    /** Base URL to serve data with. This will determine the base path and the port. */
    val baseUri: URI = URI.create("http://taskstate-service:9014"),
    /**
    ...
    fun withEnv(): TaskStateEventServerConfig = this
        .copyEnv(TASK_STATE_EVENT_SERVICE_BASE_URL) {
            try {
                copy(baseUri = URI.create(it))
            } catch (e: Exception) {
                logger.error("Not a valid url from env APPSERVER_TASK_STATE_EVENT_SERVICE_BASE_URL", e)
                this@TaskStateEventServerConfig
            }
        }
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(TaskStateEventServerConfig::class.java)
    }
}
